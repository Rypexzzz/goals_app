...
- **Строки задач в дереве — больше места под название.** Инлайн-кнопка `+` («Добавить подзадачу») переехала в меню `⋮` (рядом с Изменить/Удалить) — на её месте теперь название. **Отступ вложенности — параметр компонента**: в дереве цели лёгкий 8dp на уровень (при раскрытии через стрелку подзадачи слегка уходят вправо); **на экране задачи** все её подзадачи плоским списком без отступа — иерархия и так очевидна, мы внутри задачи. Ручка перетаскивания слегка сужена.
