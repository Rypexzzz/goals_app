...
- **Строки задач в дереве — больше места под название.** Инлайн-кнопка `+` («Добавить подзадачу») переехала в меню `⋮` (рядом с Изменить/Удалить) — на её месте теперь название. Отступ вложенности уменьшен с 16 до 10dp на уровень. Ручка перетаскивания слегка сужена. В результате на верхнем уровне в название влезает заметно больше слов, а глубокие подзадачи не съезжают вправо.
