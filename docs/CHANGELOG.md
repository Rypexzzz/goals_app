...
- **Три точки (⋮) у задачи молча удаляли её.** На экране задачи `⋮` был напрямую подключён к удалению подзадачи (`onMore = { onDeleteChild() }`) — без меню и подтверждения. Теперь `⋮` открывает нормальное меню (Изменить / Удалить) прямо в строке, а удаление проходит через диалог подтверждения. Поведение унифицировано для экранов цели и задачи; задача всё так же отправляется в Корзину (восстановимо).
