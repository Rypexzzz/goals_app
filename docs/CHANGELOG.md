...
- **Строки задач в дереве — больше места под название.** Инлайн-кнопка `+` («Добавить подзадачу») переехала в меню `⋮` (рядом с Изменить/Удалить) — на её месте теперь название. **Отступ вложенности считается от корня показанного дерева**: прямые дети идут без отступа, а то, что раскрывается через ▾, уходит вправо по 8dp на уровень. Поэтому на экране цели верхний уровень плоский, а раскрытые подзадачи смещены; на экране задачи её прямые дети плоско, а внуки — с отступом при раскрытии. Ручка перетаскивания слегка сужена.
- **Привязанные привычки на экране цели больше не прячутся за горизонтальной прокруткой.** `Row(horizontalScroll)` заменён на `FlowRow` — чипы переносятся на следующую строку, все привычки видны разом.
...
- **Вкладка «Задачи»** (2-я в нижней навигации) — плоский список всех задач с дедлайном из активных целей, без вложенности, отсортированный по близости срока: просроченные сверху и красным, выполненные скрыты. Чекбокс отмечает выполнение, тап открывает задачу. Слой: `TaskDao.observeWithDeadline` → `TaskRepository.observeTasksWithDeadline` → `GetDeadlineTasksUseCase` → `TasksViewModel`/`TasksScreen`; тест `GetDeadlineTasksUseCaseTest`.
