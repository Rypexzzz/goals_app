...
- **Пустая полоса над хэдером на всех экранах.** Корневой `Scaffold` добавлял отступ под статус-бар, а `TopAppBar` экрана — ещё раз; статус-бар учитывался дважды. Корневому каркасу выставлен `contentWindowInsets = 0` — инсеты теперь обрабатывают только топбар экрана и нижняя навигация.
