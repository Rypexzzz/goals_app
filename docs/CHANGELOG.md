...
- **Строки задач в дереве — больше места под название.** Инлайн-кнопка `+` («Добавить подзадачу») переехала в меню `⋮` (рядом с Изменить/Удалить) — на её месте теперь название. **Отступ вложенности убран полностью** — горизонтальное место отдано названию. У вложенных задач (depth > 0) слева появилась тонкая вертикальная полоска (2dp, `outlineVariant`) — чтобы они едва заметно отличались от задач верхнего уровня. Иерархия дополнительно читается через стрелку разворота и проваливание в задачу. Ручка перетаскивания слегка сужена.
